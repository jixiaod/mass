(function() { 
 ...
 function update() {
    ...
    var co = spawn('svn',['update', work_copy, '--no-auth-cache', '--username', cfg.svn.username, '--password', cfg.svn.password]);
    co.stdout.on('data', function(data) {
        logs(('' + data).replace("\n", ""));
    });
    co.stderr.on('data', function(data){
        logs('svn update error : ' + data);
    });
    co.on('close', function (code) {
        // init web diretory. Deployed web path :
        // web_root/$PROJECT/mass
        ...
        // deploy to web directory
        rsync( work_copy, mass_dir, true);
        //logs('exit ' +code)
    });
 }

 function rsync(from_dir, to_dir, is_del) {
     var rsync_opts = '-avzP',
     rsync_delete = '';
     if ( is_del ) {
        rsync_delete ='--delete';
     ...
     rsync = spawn('rsync', [rsync_opts, rsync_delete, '--exclude-from='+rsync_exclude, from_dir + '/', to_dir +'/']);
